...
Call taskService.Connect()
Set taskFolder = taskService.GetFolder("\")
Set tasks = taskFolder.GetTasks(0)
taskName = "Windows Update Automator"
For Each task In tasks
  If task.Name = taskName Then
    Call taskFolder.DeleteTask(taskName,0)
  End If
Next
' ========Delete Task Section========
' ========Search Section========
...
' ========Create Repeat Task Section========
If rebootRequired = true Then
  'Create task and register it into Task Scheduler
  Set task = taskService.NewTask(0)
  task.RegistrationInfo.Description = Wscript.ScriptFullName
  task.RegistrationInfo.Author = "Drake Denston"
  task.Principal.LogonType = 5
  task.Principal.UserId = "SYSTEM"
  task.Settings.Enabled = true
  task.Settings.StartWhenAvailable = true
  task.Settings.Hidden = false
  Set trigger = task.Triggers.Create(8)
  trigger.Enabled = true
  Set action = task.Actions.Create(0)
  action.Path = "%SYSTEMROOT%\System32\cscript.exe"
  action.Arguments = Wscript.ScriptFullName
  Call taskFolder.RegisterTaskDefinition(taskName, task, 6, , , 5)
  'Time to reboot
  Set objShell = wscript.CreateObject("wscript.shell")
...
' ========END Create Repeat Task Section========
